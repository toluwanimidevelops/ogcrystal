...
const Page = () => {
  const { getInActiveBlogs, deleteBlog, updateBlogStatus, updateBlog } =
    useApp();
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getInActiveBlogs();
      if (data?.success) {
  ...
  return (
    <div className="w-full bg-white rounded-2xl p-6   flex flex-col gap-4">
      <div className="flex items-center justify-between pb-2 border-b border-gray-100">
        <div>
          <h2 className="text-md  text-gray-800">Drafts</h2>
          <p className="text-xs text-gray-500">
            Manage, edit, and publish your unpublished articles
          </p>
        </div>
